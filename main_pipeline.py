...
def main(config_path: str):
    ...
    _data_root = os.path.join(config["paths"]["data_root"], subject_id)
    ...
    for stage_dir in [
        ...
        "stage_1_smplx_fit",
        "stage_2_textured_smplx",
        "stage_3_initial_gaussians",
        "stage_5_8_optimized_gaussians",
        "stage_9_output",
    ]:
    ...
    _images_np: list[np.ndarray] = []  # List of NumPy arrays (H, W, C) for each view
    _images_tensors: list[torch.Tensor] = (
        []
    )  # List of PyTorch tensors (C, H, W) for each view
    _num_views = 0  # Placeholder

    # === STAGE 0: PRE-PROCESSING ===
    logging.info("--- Starting Stage 0: Pre-processing ---")
    # 0.A: Human Segmentation
    _segmenter = HumanSegmenter(config["models"]["segmentation"], device=device)
    ...
    _keypoint_det = KeypointDetector(
        config["models"]["keypoint_detector"], device=device
    )
    ...
    _camera_est = CameraEstimator(
        config["tools"]["colmap_path"], config["colmap_options"]
    )
    ...
    _pixie_init = PIXIEInitializer(
        config["models"]["pixie"], config["models"]["smplx"], device=device
    )
    ...
    _smplx_refiner = SMPLXRefiner(
        config["models"]["smplx"], config["smplx_refinement_options"], device=device
    )
    ...
    logging.info(
        "--- Starting Stages 5 & 6: Gaussian Optimization & Adaptive Control ---"
    )
    ...
    logging.info(
        f"Pipeline finished for subject {subject_id}. Results in {results_root}"
    )
